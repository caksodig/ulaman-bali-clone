"use client";

import Link from "next/link";
import Image from "next/image";
import { useViewportEnter } from "@/hooks/useIntersectionObserver";
import type { RefObject } from "react";

const navigation = {
  explore: [
    { name: "Villas", href: "/villa" },
    { name: "Experience", href: "/experience" },
    { name: "Gallery", href: "/gallery" },
    { name: "About Us", href: "/about" },
  ],
  support: [
    { name: "Contact", href: "/contact" },
    { name: "Book Now", href: "/reservations" },
    { name: "FAQs", href: "/faq" },
    { name: "Sustainability", href: "/sustainability" },
  ],
  legal: [
    { name: "Privacy Policy", href: "/privacy" },
    { name: "Terms of Service", href: "/terms" },
    { name: "Cancellation Policy", href: "/cancellation" },
  ],
  social: [
    {
      name: "Instagram",
      href: "https://instagram.com/ulamanbali",
      icon: (
        <svg fill="currentColor" viewBox="0 0 24 24" className="h-6 w-6">
          <path d="M12.315 2c2.43 0 2.784.013 3.808.06 1.064.049 1.791.218 2.427.465a4.902 4.902 0 011.772 1.153 4.902 4.902 0 011.153 1.772c.247.636.416 1.363.465 2.427.048 1.067.06 1.407.06 4.123v.08c0 2.643-.012 2.987-.06 4.043-.049 1.064-.218 1.791-.465 2.427a4.902 4.902 0 01-1.153 1.772 4.902 4.902 0 01-1.772 1.153c-.636.247-1.363.416-2.427.465-1.067.048-1.407.06-4.123.06h-.08c-2.643 0-2.987-.012-4.043-.06-1.064-.049-1.791-.218-2.427-.465a4.902 4.902 0 01-1.772-1.153 4.902 4.902 0 01-1.153-1.772c-.247-.636-.416-1.363-.465-2.427-.047-1.024-.06-1.379-.06-3.808v-.63c0-2.43.013-2.784.06-3.808.049-1.064.218-1.791.465-2.427a4.902 4.902 0 011.153-1.772A4.902 4.902 0 015.45 2.525c.636-.247 1.363-.416 2.427-.465C8.901 2.013 9.256 2 11.685 2h.63zm-.081 1.802h-.468c-2.456 0-2.784.011-3.807.058-.975.045-1.504.207-1.857.344-.467.182-.8.398-1.15.748-.35.35-.566.683-.748 1.15-.137.353-.3.882-.344 1.857-.047 1.023-.058 1.351-.058 3.807v.468c0 2.456.011 2.784.058 3.807.045.975.207 1.504.344 1.857.182.466.399.8.748 1.15.35.35.683.566 1.15.748.353.137.882.3 1.857.344 1.054.048 1.37.058 4.041.058h.08c2.597 0 2.917-.01 3.96-.058.976-.045 1.505-.207 1.858-.344.466-.182.8-.398 1.15-.748.35-.35.566-.683.748-1.15.137-.353.3-.882.344-1.857.048-1.055.058-1.37.058-4.041v-.08c0-2.597-.01-2.917-.058-3.96-.045-.976-.207-1.505-.344-1.858a3.097 3.097 0 00-.748-1.15 3.098 3.098 0 00-1.15-.748c-.353-.137-.882-.3-1.857-.344-1.023-.047-1.351-.058-3.807-.058zM12 6.865a5.135 5.135 0 110 10.27 5.135 5.135 0 010-10.27zm0 1.802a3.333 3.333 0 100 6.666 3.333 3.333 0 000-6.666zm5.338-3.205a1.2 1.2 0 110 2.4 1.2 1.2 0 010-2.4z" />
        </svg>
      ),
    },
    {
      name: "Facebook",
      href: "https://facebook.com/ulamanbali",
      icon: (
        <svg fill="currentColor" viewBox="0 0 24 24" className="h-6 w-6">
          <path d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z" />
        </svg>
      ),
    },
    {
      name: "Twitter",
      href: "https://twitter.com/ulamanbali",
      icon: (
        <svg fill="currentColor" viewBox="0 0 24 24" className="h-6 w-6">
          <path d="M8.29 20.251c7.547 0 11.675-6.253 11.675-11.675 0-.178 0-.355-.012-.53A8.348 8.348 0 0022 5.92a8.19 8.19 0 01-2.357.646 4.118 4.118 0 001.804-2.27 8.224 8.224 0 01-2.605.996 4.107 4.107 0 00-6.993 3.743 11.65 11.65 0 01-8.457-4.287 4.106 4.106 0 001.27 5.477A4.072 4.072 0 012.8 9.713v.052a4.105 4.105 0 003.292 4.022 4.095 4.095 0 01-1.853.07 4.108 4.108 0 003.834 2.85A8.233 8.233 0 012 18.407a11.616 11.616 0 006.29 1.84" />
        </svg>
      ),
    },
  ],
};

export default function Footer() {
  const [ref, hasEntered] = useViewportEnter({
    threshold: 0.1,
  });

  return (
    <footer
      ref={ref as RefObject<HTMLDivElement>}
      className="bg-stone-900 text-stone-200"
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* MAIN FOOTER */}
        <div className="py-12 md:py-16 lg:py-20">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-8 lg:gap-12">
            {/* BRAND SECTION */}
            <div
              className={`lg:col-span-4 transition-all duration-700 ${
                hasEntered
                  ? "opacity-100 translate-y-0"
                  : "opacity-0 translate-y-8"
              }`}
            >
              <Link href="/" className="inline-block mb-6">
                <span className="text-3xl font-serif tracking-wider text-white">
                  ULAMAN
                </span>
              </Link>
              <p className="text-stone-400 leading-relaxed mb-6 max-w-sm">
                Experience authentic Balinese luxury in harmony with nature. A
                sustainable eco-resort committed to preserving Bali's natural
                beauty.
              </p>
              <div className="space-y-2 text-sm text-stone-400">
                <p>Jalan Raya Sanggingan, Ubud</p>
                <p>Bali 80571, Indonesia</p>
                <p className="mt-4">
                  <a
                    href="tel:+62361234567"
                    className="hover:text-white transition-colors"
                  >
                    +62 361 234 567
                  </a>
                </p>
                <p>
                  <a
                    href="mailto:hello@ulamanbali.com"
                    className="hover:text-white transition-colors"
                  >
                    hello@ulamanbali.com
                  </a>
                </p>
              </div>
            </div>

            {/* EXPLORE LINKS */}
            <div
              className={`lg:col-span-2 transition-all duration-700 ${
                hasEntered
                  ? "opacity-100 translate-y-0"
                  : "opacity-0 translate-y-8"
              }`}
              style={{ transitionDelay: "100ms" }}
            >
              <h3 className="text-sm font-medium tracking-wider uppercase text-white mb-4">
                Explore
              </h3>
              <ul className="space-y-3">
                {navigation.explore.map((item) => (
                  <li key={item.name}>
                    <Link
                      href={item.href}
                      className="text-stone-400 hover:text-white transition-colors text-sm"
                    >
                      {item.name}
                    </Link>
                  </li>
                ))}
              </ul>
            </div>

            {/* SUPPORT LINKS */}
            <div
              className={`lg:col-span-2 transition-all duration-700 ${
                hasEntered
                  ? "opacity-100 translate-y-0"
                  : "opacity-0 translate-y-8"
              }`}
              style={{ transitionDelay: "200ms" }}
            >
              <h3 className="text-sm font-medium tracking-wider uppercase text-white mb-4">
                Support
              </h3>
              <ul className="space-y-3">
                {navigation.support.map((item) => (
                  <li key={item.name}>
                    <Link
                      href={item.href}
                      className="text-stone-400 hover:text-white transition-colors text-sm"
                    >
                      {item.name}
                    </Link>
                  </li>
                ))}
              </ul>
            </div>

            {/* LEGAL LINKS */}
            <div
              className={`lg:col-span-2 transition-all duration-700 ${
                hasEntered
                  ? "opacity-100 translate-y-0"
                  : "opacity-0 translate-y-8"
              }`}
              style={{ transitionDelay: "300ms" }}
            >
              <h3 className="text-sm font-medium tracking-wider uppercase text-white mb-4">
                Legal
              </h3>
              <ul className="space-y-3">
                {navigation.legal.map((item) => (
                  <li key={item.name}>
                    <Link
                      href={item.href}
                      className="text-stone-400 hover:text-white transition-colors text-sm"
                    >
                      {item.name}
                    </Link>
                  </li>
                ))}
              </ul>
            </div>

            {/* NEWSLETTER */}
            <div
              className={`lg:col-span-2 transition-all duration-700 ${
                hasEntered
                  ? "opacity-100 translate-y-0"
                  : "opacity-0 translate-y-8"
              }`}
              style={{ transitionDelay: "400ms" }}
            >
              <h3 className="text-sm font-medium tracking-wider uppercase text-white mb-4">
                Newsletter
              </h3>
              <p className="text-sm text-stone-400 mb-4">
                Subscribe for exclusive offers and updates.
              </p>
              <form className="space-y-3">
                <input
                  type="email"
                  placeholder="Your email"
                  className="w-full px-4 py-2 bg-stone-800 border border-stone-700 text-white placeholder-stone-500 focus:outline-none focus:border-ulaman-gold transition-colors text-sm"
                />
                <button
                  type="submit"
                  className="w-full px-4 py-2 bg-white text-stone-900 hover:bg-stone-100 transition-colors text-sm tracking-wider uppercase font-medium"
                >
                  Subscribe
                </button>
              </form>
            </div>
          </div>
        </div>

        {/* DIVIDER */}
        <div className="border-t border-stone-800" />

        {/* BOTTOM FOOTER */}
        <div
          className={`py-8 transition-all duration-700 ${
            hasEntered ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
          }`}
          style={{ transitionDelay: "500ms" }}
        >
          <div className="flex flex-col md:flex-row justify-between items-center gap-6">
            {/* COPYRIGHT */}
            <p className="text-sm text-stone-500 text-center md:text-left">
              © {new Date().getFullYear()} Ulaman Bali. All rights reserved.
            </p>

            {/* SOCIAL LINKS */}
            <div className="flex items-center space-x-6">
              {navigation.social.map((item) => (
                <a
                  key={item.name}
                  href={item.href}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-stone-400 hover:text-white transition-colors"
                  aria-label={item.name}
                >
                  {item.icon}
                </a>
              ))}
            </div>

            {/* CERTIFICATIONS (Optional) */}
            <div className="flex items-center space-x-4">
              <span className="text-xs text-stone-500 tracking-wider uppercase">
                Certified Eco-Resort
              </span>
            </div>
          </div>
        </div>
      </div>
    </footer>
  );
}
